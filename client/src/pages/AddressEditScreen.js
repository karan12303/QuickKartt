...
const AddressEditScreen = () => {
  ...
  // Load Google Maps API with hardcoded key for now
  ...
  }, [navigate, userInfo, id, isNewAddress]);
  ...
  const submitHandler = async (e) => {
    ...
    try {
      ...
      if (isNewAddress) {
        ...
        response = await axios.post('/api/users/addresses', addressData, config);
        console.log('Response:', response.data);
      } else {
        console.log('Updating address:', id);
        response = await axios.put(`/api/users/addresses/${id}`, addressData, config);
        console.log('Response:', response.data);
      }
  ...
  return (
    <>
      ...
      <FormContainer>
        <h1>{isNewAddress ? 'Add New Address' : 'Edit Address'}</h1>
        {loading ? (
          <Loader />
        ) : error ? (
